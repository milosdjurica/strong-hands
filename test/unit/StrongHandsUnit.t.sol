...
contract StrongHandsUnitTest is SetupTestsTest {
    ...
    function test_dividendsOwing() public {
        ...
        // ! Alice deposits and withdraws and pays 1 ether penalty
        vm.startPrank(ALICE);
        harness.deposit{value: 2 ether}();

        // ! Check harness
        assertEq(harness.totalStaked(), 3 ether);
        assertEq(harness.unclaimedDividends(), 0 ether);
        assertEq(harness.totalDividendPoints(), 0);

        harness.withdraw();
        vm.stopPrank();

        assertEq(harness.totalDividendPoints(), 1e54);

        // ! Check Bob
        (uint256 balanceBobAfter, uint256 timestampBobAfter, uint256 lastDividendPointsBobAfter) = harness.users(BOB);
        assertEq(balanceBobAfter, 1 ether);
        assertEq(timestampBobAfter, block.timestamp);
        assertEq(lastDividendPointsBobAfter, 0 ether);

        // ! Check harness
        assertEq(harness.totalStaked(), 2 ether);
        assertEq(harness.unclaimedDividends(), 1 ether);
        assertEq(harness.totalDividendPoints(), 1e54);
        ...
        depositWith(BOB, 2 ether)
    {
        ...
        // ! Check StrongHands
        assertEq(strongHands.totalStaked(), 2 ether); // Alice 1
        assertEq(strongHands.unclaimedDividends(), 1 ether); // 1 Bob first penalty that Alice didn't claim
        assertEq(strongHands.totalDividendPoints(), 1e54);
        ...
        assertEq(balance, 1 ether);
        assertEq(timestamp, block.timestamp);
        assertEq(lastDividendPoints, 1e54);

        // ! Check StrongHands
        assertEq(strongHands.totalStaked(), 3 ether); // Alice 1 + Bob Bob penalty + Bob 1 later
        assertEq(strongHands.unclaimedDividends(), 1 ether); // 1 Bob first penalty that Alice didn't claim
        assertEq(strongHands.totalDividendPoints(), 1e54);
    }
    ...
    function test_withdraw_MidPenalty_AliceClaims() public depositWith(ALICE, 1 ether) depositWith(BOB, 1 ether) {
        skip(LOCK_PERIOD / 2);
        // ! Bob withdraws and pays 25% penalty
        vm.prank(BOB);
        vm.expectEmit(true, true, true, true);
        emit Withdrawn(BOB, 0.75 ether, 0.25 ether, block.timestamp);
        strongHands.withdraw();
